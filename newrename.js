...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    if (findKey?.[1]) {
      ...
      // 构建名称时，将 diyAndPort 放在最后，并用 FGF 分隔
      let newBaseName = keyover.join(FGF);
      if (diyAndPort) {
          e.name = `${newBaseName}${FGF}${diyAndPort}`;
      } else {
          e.name = newBaseName;
      }
    } else {
      if (nm) {
        ...
        // 如果保留未匹配节点，并且有 diyAndPort，则附加
        if (diyAndPort) {
             e.name = `${e.name}${FGF}${diyAndPort}`;
        }
      } else {
        e.name = null;
      }
